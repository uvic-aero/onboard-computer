...
class BaseCameraAPI:
    ...
    def _send_command(self, method, callback, params):
        ...
        # images
        try:
            res = requests.post(self.url, json=self.payload, timeout=3.5)
        except:
...
class CameraAPI(BaseCameraAPI):
    ...
    def still_capture(self, callback=None):
        self._queue_command(
            "actTakePicture",
            callback if callback is not None else self._still_capture_result,
        )
    ...
    def check_status(self, callback=None):
        self._queue_command(
            "getEvent",
            callback if callback is not None else self._check_status_result,
            params=[False],
        )
