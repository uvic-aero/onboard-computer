...
class Camera:
    def __init__(self):
        ...
        self.endpoint = ""
        self.params = {"id": 1, "version": "1.0", "params": [], "method": ""}
        self.liveview = None
    ...
    async def parse_ssdp_response(self):
        ...
        for service in root.iter("{urn:schemas-sony-com:av}X_ScalarWebAPI_Service"):
            if (
                service.find("{urn:schemas-sony-com:av}X_ScalarWebAPI_ServiceType").text
                == "camera"
            ):
                self.endpoint = (
                    service.find(
    ...
    async def send_command(self, method, param=None):
        ...
        try:
            res = await ioloop.IOLoop.instance().asyncio_loop.run_in_executor(
                None, functools.partial(requests.post, self.endpoint, json=self.params)
            )
